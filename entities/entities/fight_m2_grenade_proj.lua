AddCSLuaFile()
ENT.Type = "anim"
ENT.Base = "fight_base_grenade_proj"
...
local function PushPullRadius(pos, pusher)
    ...
    for k, target in ipairs(ents.FindInSphere(pos, radius)) do
        if IsValid(target) then
            ...
            local height_diff = pos.z - tpos.z
            local dir = (tpos - pos):GetNormal()
            local phys = target:GetPhysicsObject()

            local isNonfrozenPlayer = target:IsPlayer() and not target:IsFrozen()
            local isNpc = target:IsNPC()

            if (isNonfrozenPlayer or isNpc) and ((not target.was_pushed) or target.was_pushed.t ~= CurTime()) then
                -- always need an upwards push to prevent the ground's friction from
                -- stopping nearly all movement
                if height_diff < 30 then
...
function ENT:Explode(tr)
    if SERVER then
        ...
        self:Remove()
        baseclass.Get("player_fight_merc").StartRegen(self:GetThrower())
